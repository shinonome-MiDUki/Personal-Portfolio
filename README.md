...
- **インフラ（サーバー）** 
  - Docker (習熟度：⭐️⭐️⭐️)
  - クラウド（AWS等） (習熟度：⭐️⭐️)
  - Linux（Ubuntu） (習熟度：⭐️⭐️⭐️)
- **GUIフレームワーク**
  - PySide/PyQt (習熟度：⭐️⭐️⭐️⭐️)
- **開発ツール**
  - Git (習熟度：⭐️⭐️⭐️⭐️)
  - Perforce (習熟度：⭐️⭐️⭐️)
...
大学では、クリエーター向けツール開発とデジタルコンテンツ制作インフラ整備に力入れてまいりました。以下はこれまでの開発実績の一部です。

全てのプロジェクトは[GitHub ページ](https://github.com/shinonome-MiDUki)をご参照ください。

- **クリエーターのためのリファレンスコレクションツール** - [Oxoria](#oxoria)
  - [UI紹介](#ui紹介)
  - [リファレンスライブラリ](#リファレンスライブラリ)
  - [セマンティック検索](#セマンティック検索)
  - [Python APIサポートとスクリプトによるカスタマイズ](#スクリプトによるカスタマイズ)
  - [ヘッドレス操作](#ヘッドレス操作)
  - [重複検出](#重複検出)
- **セルアニメの制作パイプラインを支える** - [ShellArc](#shellarc)
  - [105人のアニメーション企画の基盤パイプラインとしてのShellArc](#105人のアニメーション企画の基盤パイプラインとしてのshellarc)
  - [Gitを用いた強力なバージョン管理機能とユーザー体験の両立](#gitを用いた強力なバージョン管理機能とユーザー体験の両立)
  - [四回にわたるリファクタリング](#四回にわたるリファクタリング)
  - [現場のためのツールを目指して](#現場のためのツールを目指して)
  - [クリエーターファーストのアプローチ](#クリエーターファーストのアプローチ)
  - [ユーザーサポートの工夫](#ユーザーサポート)
  - [オープンソース化](#オープンソース化)
- **アニメコマ打ちとタイムライン操作に特化したツール** - [Shell DELTA](#shell-delta)
  - [UI紹介](#ui紹介-1)
  - [クリーンコードへの配慮](#クリーンコードへの配慮)
  - [GPUを駆使した描画](#gpuを駆使した描画)
  - [エクスプレッションシステム](#エクスプレッションシステム)
  - [今後について](#今後について)
- **GPUネイティブの高速映像規格** - [NUDEC](#nudec)
  - [コンセプト](#コンセプト)
  - [パイプライン](#パイプライン)
- **2Dアートワーク** - [2Dアートワーク](#2dアートワーク)
...
Discord ボットコマンドリファレンス：[ShellArc Discord](https://qu-miteiken-shelltech.github.io/Shell_Arc/#errcode)
...
## 105人のアニメーション企画の基盤パイプラインとしてのShellArc
...
#### Discord ボットの使用例
![Discord ボット](./img/shellarc_dc.jpg)

#### ShellArc Desktop アプリの使用例
![Discord Desktop](./img/shellarc_desktop.jpg)
...
「わからない」ことが作業の妨げにならないように、ユーザーサポートに心がけています

1. **生成AI活用**：![チャットボット](./img/shellarc_nuru.jpg)
RAG技術を活用し、AIテクニカルサポートボットを構築し提供しています
2. **コマンド生成**：複雑なSAPYCコマンドを自動生成できる生成器をドキュメントページに埋め込んでいます
3. **専用サポートルーム**：![SAPYC生成器](./img/shellarc_sapycgen.jpg)
Discordの制作チャンネルに ShellArc のテクニカルサポート専用のチャンネルを作成してあります
...
PySide6（GUIフレームワーク）とOpenGLをベースとしたソフトウェアです。
...
#### 開発中バージョン
![Shell_DELTA_newUI](./img/shell_delta_newui.jpg)

7. レイヤーエディタ

## クリーンコードへの配慮

コードと**関数責務の分離**を徹底し、どの機能のコードをどこで参照できるかがすぐわかるようなコード構成に心がけています。
...
大量な連番画像を安定して30fpsでリアルタイムで再生するために、**標準の QImage ではなく、OpenGL を用いてより低レベルから再生システムを実装しました。**また、レイヤー構造は OpenGL の GLSL fragment shader で実現しています。
...
3. **レイヤー・タイムシート対応**：CSV形式のタイムシートに対応する予定です。マルチレイヤーエディタはまだ新機能なため、デバッグとUI/UX改善が必要です
4. **ストリーミングロード実装**：連番画像が膨大になると、物量的にVRAMが限界を迎え、オーバーフローしてしまいますので、連番画像をチャンクに分けて、バックグラウンドストリーミングロードで解決していきたい
5. **例外処理**：現在、「仕様書通りに操作したら動作する」ソフトウェアになっていますが、想定外の操作や誤操作などにまだ弱いため、より強力で包括的な例外処理が必要です
...
リポジトリ：[NUDEC](https://github.com/QU-miteiken-shelltech/Shell_NUDEC)

Shell DELTA と同じく、早期開発中のプロジェクトです。
...
**CLIP STUDIO PAINT**を用いて制作した作品です

- アイコンのキャラクターデザイン![イコンのキャラクターデザイン](./2dart/drawing000.jpg)
- 実写写真と手描きイラストを合わせた作品![実写写真と手描きイラストを合わせた作品](./2dart/drawing001.jpg)
- 一枚絵![一枚絵](./2dart/drawing002.jpg)
- 円形のスペースに描いた作品![円形のスペースに描いた作品](./2dart/drawing003.jpg)